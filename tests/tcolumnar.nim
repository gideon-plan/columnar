...
suite "schema":
  test "create schema":
    let s = schema(column("id", ctInt64), column("name", ctUtf8))
...
suite "arrow":
  ...
  test "float column":
    let s = schema(column("val", ctFloat64))
...
suite "convert":
  ...
  test "round-trip table -> batch -> table":
    let s = schema(column("x", ctInt64), column("y", ctFloat64))
    var rows: seq[Table[string, string]]

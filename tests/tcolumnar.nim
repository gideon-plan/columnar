...
suite "schema":
  test "create schema":
    let s = schema(column("id", ColumnType.Int64), column("name", ColumnType.Utf8))
...
suite "arrow":
  ...
  test "float column":
    let s = schema(column("val", ColumnType.Float64))
...
suite "parquet":
  test "parquet file structure":
    let s = schema(column("x", ColumnType.Int64))
...
suite "convert":
  ...
  test "round-trip table -> batch -> table":
    let s = schema(column("x", ColumnType.Int64), column("y", ColumnType.Float64))
    var rows: seq[Table[string, string]]
